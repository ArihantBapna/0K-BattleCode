...
public class Scan {
    ...
    public int GetNearbyFriendlyEC() throws GameActionException{
        ec = new ArrayList<>();
    ...
    public boolean IsECSurrounded() throws GameActionException{
        MapLocation m = ec.get(0).getLocation();
    ...
    public int GetNearbyFriendlyMuck() throws GameActionException{
        mucks = new ArrayList<>();
        for(RobotInfo r : rc.senseNearbyRobots(-1,rc.getTeam())){
            if(r.getType().equals(RobotType.MUCKRAKER)){
                mucks.add(r);
